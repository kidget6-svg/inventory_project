...
export default function CashierPaymentQueue({ saleType, showTypeTabs = false }) {const { t } = useLanguage();
  ...
  const fetchPendingSales = async () => {
    ...
    try {
      ...
      window.showToast(t("Failed to fetch cashier queue"), 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleTelebirrPay = () => {
    if (!telebirrPhone.trim()) {
      window.showToast(t("Customer phone number is required"), 'error');
      return;
    }
  ...
  const handleCompleteSale = async () => {
    ...
    if (isCash && paid < total) {
      window.showToast(t("Amount paid cannot be less than the total amount"), 'error');
      return;
    }

    // For Telebirr, ensure the payment has been processed
    if (paymentMethod === 'telebirr' && telebirrStep !== 'verify') {
      window.showToast(t("Please complete the Telebirr payment first"), 'error');
      return;
    }
    ...
    try {
      ...
      window.showToast(t("Payment completed successfully!"), 'success');
    } catch (err) {
  ...
  if (loading) return <LoadingSpinner text={t("Fetching cashier queue...")} />;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
          ...
          className="btn-secondary px-4 py-2 text-sm flex items-center gap-2 whitespace-nowrap">
                    ...
                    <RefreshCw size={16} />{t("Refresh Queue")}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col">
                            ...
                            <div className="px-5 py-4 border-b border-gray-100">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <span className="text-xs font-medium text-gray-400 uppercase tracking-wide">{t("Order Ref")}
                ...
                'bg-emerald-50 text-emerald-700 border border-emerald-100'}`
                                        ...
                                        <span className="px-2.5 py-1 bg-amber-50 text-amber-700 border border-amber-100 rounded-full text-xs font-semibold whitespace-nowrap">
                                            <Clock size={12} className="inline mr-1" />{t("Pending Cashier")}
                            ...
                            <div className="px-5 py-4 flex-1 space-y-4">
                                {/* Line Items */}
                                <div>
                                    <label className="block text-xs font-semibold text-gray-500 mb-2">{t("Line Items (")}
                {sale.items?.length || 0})
                                    </label>
                  ...
                  defaultValue="">
                                            ...
                                            <option value="" disabled>{t("Click to view full breakout...")}

                  </option>
                                            {sale.items?.map((item, idx) =>
                  <option key={idx} value={item.id} disabled>
                                                    {item.medicine?.name || item.product_name || `Item #${item.medicine_id}`}{t("| Qty:")}{item.quantity} | ${parseFloat(item.unit_price).toFixed(2)} ea
                                                </option>
                  )}
                                        </select>
                                ...
                                <div className="flex items-baseline justify-between pt-2">
                                    <span className="text-xs font-medium text-gray-500">{t("Total Amount")}</span>
            ...
            <div className="flex justify-between text-xs text-gray-500">
                                        <span>{t("Discount (")}{sale.discount_type === 'percentage' ? `${sale.discount}%` : 'Fixed'})</span>
            ...
            <div className="text-center text-xs text-gray-400 py-2">{t("View only")}</div>
            }
                            </div>
                        </div>
        )}
        ...
        <div className="space-y-4">
                        <div className="bg-gray-50 rounded-lg p-4">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-xs text-gray-500">{t("Order Ref")}</span>
            ...
            <div className="flex justify-between items-center mb-2">
                        ...
                        <div>
                            <label className="block text-xs font-medium text-gray-500 mb-2">{t("Select Payment Method")}

            </label>
                            <div className="grid grid-cols-2 gap-2">
                                {PAYMENT_METHODS.map((pm) => {const { t } = useLanguage();
                const Icon = pm.icon;
                return (
                  <button
                    key={pm.value}
                    ...
                    'bg-gray-50 text-gray-700 hover:bg-gray-100 border border-gray-200'}`
                        ...
                        {paymentMethod === 'cash' &&
          <div>
                                <label className="block text-xs font-medium text-gray-500 mb-1">{t("Amount Paid")}
              ...
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-sky-500 outline-none" />
                                ...
                                {changeAmount > 0 &&
            <div className="mt-2 text-sm text-gray-600">{t("Change to return:")}
          ...
          <div className="space-y-3">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-xs font-medium text-green-700">{t("Verification code sent to")}
                  {telebirrPhone}
                                            </span>
                                        </div>
                                        ...
                                        <p className="text-xs text-gray-500 mt-2 text-center">{t("Enter this code on the customer's Telebirr app to complete the payment.")}
              ...
              className="flex-1 btn-secondary px-4 py-2 text-sm">{t("Cancel")}
        ...
        <div className="space-y-4">
                        ...
                        <div className="text-center">
                            <h3 className="text-lg font-bold text-gray-800 mb-1">{t("\u2713 Payment Successful")}</h3>
                            <p className="text-sm text-gray-500">{t("The sale has been completed successfully.")}</p>
                        </div>
