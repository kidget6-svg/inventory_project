...
export default function Pagination({ meta, onPageChange }) {const { t } = useLanguage();
  if (!meta) return null;
  ...
  return (
    <div className="mt-4 flex items-center justify-between">
            <div className="text-sm text-gray-600 dark:text-gray-400">{t("Showing")}{start} to {end} of {total} results</div>
            <div className="inline-flex items-center space-x-1">
                <button
          onClick={() => onPageChange(Math.max(1, current_page - 1))}
          disabled={current_page === 1}
          className={`px-3 py-1 rounded ${current_page === 1 ? 'text-gray-400 bg-gray-100 dark:bg-gray-800 dark:text-gray-600' : 'text-sky-600 bg-white hover:bg-sky-50 dark:bg-gray-800 dark:text-sky-400 dark:hover:bg-gray-700 dark:border dark:border-gray-700'} text-sm`}>{t("Previous")}
        ...
        <button onClick={() => onPageChange(last_page)} className="px-3 py-1 rounded text-sm text-sky-600 bg-white hover:bg-sky-50 dark:bg-gray-800 dark:text-sky-400 dark:hover:bg-gray-700 dark:border dark:border-gray-700">{last_page}</button>
          ...
          className={`px-3 py-1 rounded ${current_page === last_page ? 'text-gray-400 bg-gray-100 dark:bg-gray-800 dark:text-gray-600' : 'text-sky-600 bg-white hover:bg-sky-50 dark:bg-gray-800 dark:text-sky-400 dark:hover:bg-gray-700 dark:border dark:border-gray-700'} text-sm`}>{t("Next")}
