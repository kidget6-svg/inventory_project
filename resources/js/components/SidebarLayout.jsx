...
import {
    ...
    Bell, ClipboardList, TrendingUp, AlertTriangle, Store, GitBranch,
    Globe, Layers, Zap, Shield, UserCheck, Activity, Home, FileText as FileIcon
} from 'lucide-react';
...
const menuItems = [
    ...
    // Product Management (Categories first, then Medicines, then Retail Products)
    ...
    // Inventory & Warehousing (Warehouse first, Branches, Stock Movements, Stock Management)
    ...
    { to: '/profile', label: 'Profile', icon: UserCircle, permissions: ['profile.view'] },
...
export default function SidebarLayout({ children, pageTitle }) {
    ...
    return (
        <div className="flex min-h-screen bg-gray-50">
            ...
            {/* Sidebar */}
            <aside
                className={`fixed top-0 left-0 w-64 ${sidebarWidth} h-screen bg-[#E3F2FD] border-r border-sky-200 z-40 flex flex-col overflow-hidden transition-all duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
                    } md:translate-x-0`}
            >
                {/* Logo & Brand */}
                <div className="p-4 border-b border-sky-200 flex items-center justify-between">
                    ...
                    <button
                        ...
                        className={`hidden md:flex shrink-0 items-center justify-center w-7 h-7 rounded-lg text-gray-600 hover:bg-sky-200 hover:text-gray-900 transition-colors ${collapsed ? 'md:absolute md:top-4 md:right-[-14px] bg-[#E3F2FD] border border-sky-200' : ''
                            }`}
                ...
                {/* User Info - shown when collapsed */}
                {collapsed && (
                    <div className="flex flex-col items-center py-3 border-b border-sky-200">
                        ...
                        <div className="text-[10px] text-gray-600 mt-1 text-center font-medium truncate w-full px-2">
            ...
            <main className={`flex-1 ${mainMargin} min-h-screen transition-all duration-300`}>
                {/* Top Bar */}
                <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-sky-200/60 px-6 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <h2 className="text-xl font-bold text-gray-900 tracking-tight">{pageTitle || 'Dashboard'}</h2>
                        {/* Branch indicator for non-admin */}
                        {user && user.role !== 'admin' && user.branch && (
                            <span className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium text-sky-700 bg-sky-50 rounded-full border border-sky-200">
                                <Store size={12} />
                                {user.branch.name}
                            </span>
                        )}
                            ...
                            <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-sky-100 py-2 z-50">
                                <div className="px-4 py-2 border-b border-sky-100 mb-1">
                                    ...
                                    <div className="flex items-center gap-1.5 mt-1">
                                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-semibold uppercase tracking-wider ${roleBadgeStyle[user?.role] || 'bg-gray-100 text-gray-600'}`}>
                                            {user?.role || 'Guest'}
                                        </span>
