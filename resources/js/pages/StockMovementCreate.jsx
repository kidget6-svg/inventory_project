import { useLanguage } from "../context/LanguageContext";import React, { useState, useEffect, useMemo, useRef } from 'react';
...
import {
  ...
  ShoppingBag, ChevronDown, Check, Warehouse } from
'lucide-react';
...
function SearchableSelect({ items, value, onSelect, placeholder, icon: Icon, accent = 'sky' }) {const { t } = useLanguage();
    ...
    <div className="relative" ref={ref}>
        ...
        className="w-full flex items-center gap-3 px-4 py-3 border border-gray-200 rounded-xl text-sm bg-white hover:border-sky-300 transition-colors text-left">
          ...
          <>
                            ...
                            <p className="text-xs text-gray-500">{t("Stock:")}{selected.stock ?? 0}{selected.sub ? ` | ${selected.sub}` : ''}</p>
                        </> :
      ...
      <div className="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden">
            ...
            placeholder={t("Search by name, generic name, SKU, or barcode...")}
            className="w-full pl-10 pr-3 py-2.5 text-sm outline-none" />
          
                    </div>
                    <div className="max-h-64 overflow-y-auto">
                        {filtered.length === 0 ?
          <div className="px-4 py-8 text-center text-gray-400 text-sm">{t("No products found")}
            {query ? ` for "${query}"` : ''}
                            </div> :
          filtered.map((item) =>
          <button
            ...
            value === item.value ? 'bg-sky-50' : ''}`
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs text-gray-500 truncate">{t("Stock:")}
...
/* ── Main Component ──────────────────────────────────────── */
export default function StockMovementCreate() {const { t } = useLanguage();
  ...
  useEffect(() => {
    ...
    catch(() => setError(t("Failed to load products"))).
  ...
  const handleSubmit = async (e) => {
    ...
    if (items.length === 0) {
      setError(t("Please add at least one product."));
      return;
    }
    if (!movementType) {
      setError(t("Please select a movement type."));
      return;
    }
    ...
    try {
      ...
      window.showToast(t("Stock movements recorded successfully"), 'success');
  ...
  if (loading) return <LoadingSpinner text={t("Loading products...")} />;
  ...
  return (
    <div className="space-y-6 max-w-5xl">
          ...
          className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-colors">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">{t("Record Stock Movement")}</h1>
                    <p className="text-sm text-gray-500 mt-0.5">{t("Add multiple items and record a single movement type")}</p>
                </div>
            </div>
      ...
      <div className="bg-red-50 border border-red-100 text-red-700 p-4 rounded-xl text-sm flex items-center gap-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm space-y-5">
                    <h3 className="font-bold text-gray-800 flex items-center gap-2">
                        <Package size={18} className="text-sky-600" />{t("1. Add Products")}
              ...
              'text-gray-500 hover:text-gray-700'}`
                            ...
                            <ShoppingBag size={16} />{t("Retail & OTC")}
            ...
            <div className="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg max-h-64 overflow-y-auto">
              ...
              <div className="px-4 py-6 text-center text-gray-400 text-sm">{t("No products found")}</div> :
              filteredProducts.map((p) => {const { t } = useLanguage();
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-gray-50">
                                        ...
                                        <th className="px-3 py-2 text-center text-xs font-semibold text-gray-600 w-24">{t("Type")}</th>
                                        <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600">{t("Manufacturer")}</th>
                                        <th className="px-3 py-2 text-right text-xs font-semibold text-gray-600 w-28">{t("Quantity")}</th>
                                        <th className="px-3 py-2 text-right text-xs font-semibold text-gray-600 w-24">{t("Stock")}</th>
          ...
          <div className="text-center py-8 border-2 border-dashed border-gray-200 rounded-xl text-gray-400 text-sm">{t("No products added yet. Search and add products above.")}
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm space-y-5">
                    <h3 className="font-bold text-gray-800 flex items-center gap-2">
                        <ClipboardList size={18} className="text-sky-600" />{t("2. Movement Type")}

          </h3>
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                        {movementTypes.map((mt) => {const { t } = useLanguage();
              const Icon = mt.icon;
              return (
                <button
                  key={mt.value}
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm space-y-5">
                    <h3 className="font-bold text-gray-800 flex items-center gap-2">
                        <FileText size={18} className="text-sky-600" />{t("3. Details & Confirm")}
            ...
            <div>
                ...
                <div className="flex items-center justify-between pt-2">
                    ...
                    <div className="flex items-center gap-3">
                        <span className="text-xs text-gray-500">{items.length}{t("item(s)")}</span>
                        <button
              type="submit"
              ...
              className="px-6 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-sky-600 to-blue-600 rounded-xl shadow-lg shadow-sky-500/20 hover:shadow-xl transition-all duration-300 flex items-center gap-2 disabled:opacity-60">
                            ...
                            {submitting ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Recording...</> : <><Save size={16} />{t("Record Movements")}</>}
                        </button>
                    </div>
                </div>
            </form>
