...
import {
  ...
  Calendar, AlertOctagon, CheckCircle, ArrowUpCircle } from
'lucide-react';
...
// MAIN COMPONENT
// ============================================================
export default function LowStock() {const { t } = useLanguage();
  ...
  const loadData = useCallback(() => {
    ...
    then((r) => {
      ...
      console.error(err);
      setError(t("Failed to load low stock data"));
    }).
  ...
  const getStatusBadge = useCallback((medicine) => {const { t } = useLanguage();
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
                <div className="text-center">
                    ...
                    <p className="text-gray-500">{t("Loading inventory data...")}</p>
                </div>
            </div>);
  ...
  if (error) {
    return (
      <div className="bg-white rounded-2xl border border-red-200 p-12 text-center">
                ...
                <h3 className="text-lg font-semibold text-gray-800 mb-2">{t("Something went wrong")}</h3>
                <p className="text-gray-500 mb-4">{error}</p>
                <button onClick={loadData} className="btn-primary px-6 py-2">{t("Retry")}</button>
            </div>);
    ...
    <div className="space-y-6 pb-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
                        <AlertTriangle className="w-7 h-7 text-amber-500" />{t("Low Stock Management")}

          </h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400">{t("Monitor inventory health with visual metrics")}</p>
                </div>
           ...
           ============================================================ */}
            ...
            strokeWidth={10} />
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-400 mt-2">{t("All medicines in inventory")}</p>
                </div>
          ...
          onClick={() => handleDonutClick('outOfStock')}>
            ...
            active={filters.status === 'out'} />
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-400 mt-2">{metrics.outOfStockPercentage.toFixed(1)}{t("% of total")}</p>
                </div>
          ...
          onClick={() => handleDonutClick('lowStock')}>
            ...
            active={filters.status === 'low'} />
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-400 mt-2">{metrics.lowStockPercentage.toFixed(1)}{t("% of total")}</p>
                </div>
            ...
            strokeWidth={10} />
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-400 mt-2">{t("Total inventory value")}</p>
                </div>
          ...
          onClick={() => handleDonutClick('expired')}>
            ...
            active={filters.status === 'expired'} />
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-400 mt-2">{metrics.expiredPercentage.toFixed(1)}{t("% of total")}</p>
                </div>
            </div>
           ...
           ============================================================ */}
            <div className="sticky top-0 z-20 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border border-gray-200 dark:border-gray-700 rounded-2xl p-4 shadow-sm">
              ...
              type="text"
              name="search"
              placeholder={t("Search medicine, generic, barcode...")}
              ...
              className="w-full pl-9 pr-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-100 dark:focus:ring-sky-900/30 outline-none bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500" />
                    ...
                    <select name="category_id" value={filters.category_id} onChange={handleFilterChange} className="px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm outline-none min-w-[140px] bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
                        <option value="" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{t("All Categories")}</option>
                    ...
                    <select name="supplier_id" value={filters.supplier_id} onChange={handleFilterChange} className="px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm outline-none min-w-[140px] bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
                        <option value="" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{t("All Suppliers")}</option>
                    ...
                    <select name="status" value={filters.status} onChange={handleFilterChange} className="px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm outline-none min-w-[130px] bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
                        <option value="" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{t("All Status")}</option>
                        <option value="critical" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{t("Critical")}</option>
                        <option value="low" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{t("Low Stock")}</option>
                        <option value="out" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{t("Out of Stock")}</option>
                        <option value="expired" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{t("Expired")}</option>
                    </select>
                    <select name="sort" value={filters.sort} onChange={handleFilterChange} className="px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm outline-none min-w-[140px] bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
                        <option value="name_asc" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{t("Name A-Z")}</option>
                        <option value="name_desc" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{t("Name Z-A")}</option>
                        <option value="quantity_asc" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{t("Stock Low-High")}</option>
                        <option value="quantity_desc" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white">{t("Stock High-Low")}</option>
                    </select>
                    {isFiltered &&
            ...
            <div ref={tableRef} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full min-w-[1000px]">
                        <thead className="bg-gradient-to-r from-sky-600 to-blue-600 text-white">
                            <tr>
                                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">{t("Medicine")}</th>
                                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">{t("Category")}</th>
                                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider">{t("Qty")}</th>
                                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider">{t("Reorder")}</th>
                                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider">{t("Status")}</th>
                                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">{t("Expiry")}</th>
                                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider">{t("Supplier")}</th>
                                <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{t("Actions")}</th>
                            </tr>
                        </thead>
              ...
              <tr>
                                    <td colSpan={8} className="px-4 py-12 text-center">
                                        ...
                                        <p className="text-gray-800 dark:text-gray-200 font-semibold">{t("All Stocked Up!")}</p>
                                        <p className="text-sm text-gray-500 dark:text-gray-400">{t("No medicines match your current filters.")}</p>
                                    </td>
                                </tr> :
              filteredMedicines.map((medicine) => {const { t } = useLanguage();
                const status = getStockStatus(medicine);
                return (
                  <tr key={medicine.id} className="hover:bg-sky-50/30 dark:hover:bg-gray-700/50 transition-colors cursor-pointer" onClick={() => {setSelectedMedicine(medicine);setShowModal(true);}}>
                                        ...
                                        <td className="px-4 py-3 text-right">
                                            <div className="flex justify-end gap-1">
                                                <button onClick={(e) => {e.stopPropagation();setSelectedMedicine(medicine);setShowModal(true);}} className="p-1.5 text-sky-600 hover:bg-sky-50 rounded-lg transition-colors" title={t("View")}>
                                                    <Eye size={16} />
                                                </button>
                                                {canOrder &&
                        <button onClick={(e) => {e.stopPropagation();handleOrderNow(medicine.id);}} disabled={ordering === medicine.id} className="p-1.5 text-sky-600 hover:bg-sky-50 rounded-lg transition-colors disabled:opacity-50" title={t("Reorder")}>
        ...
        <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                                <p className="text-xs text-gray-500 dark:text-gray-400">{t("Generic Name")}</p>
                            ...
                            <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                                <p className="text-xs text-gray-500 dark:text-gray-400">{t("Current Stock")}</p>
                                <p className="text-sm font-bold text-gray-800 dark:text-white">{selectedMedicine.quantity || 0}</p>
                            </div>
                            <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                                <p className="text-xs text-gray-500 dark:text-gray-400">{t("Reorder Level")}</p>
                                <p className="text-sm font-medium text-gray-800 dark:text-white">{selectedMedicine.reorder_level || 0}</p>
                            </div>
                            <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                                <p className="text-xs text-gray-500 dark:text-gray-400">{t("Expiry Date")}</p>
            ...
            <button onClick={() => {handleOrderNow(selectedMedicine.id);}} className="flex-1 btn-primary py-2.5 text-sm flex items-center justify-center gap-2">
                                    <ShoppingCart size={16} />{t("Order Now")}
            </button>
            }
                        </div>
                    </div>
