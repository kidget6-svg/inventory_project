...
export default function SupplierView() {const { t } = useLanguage();
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    catch(() => setError(t("Unable to load supplier details."))).
    finally(() => setLoading(false));
  }, [id]);

  const handleDelete = async () => {
    if (!canDelete) {
      window.showToast(t("You do not have permission to delete suppliers"), 'error');
      return;
    }
  ...
  if (!supplier) {
    return <div className="min-h-[60vh] p-6">{t("Supplier not found.")}</div>;
  }
  ...
  return (
    <div className="space-y-6">
          ...
          className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                ...
                <h1 className="text-2xl font-bold text-gray-800">{t("Supplier Details")}</h1>
            </div>
