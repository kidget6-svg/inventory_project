...
export default function RetailProducts() {const { t } = useLanguage();
  ...
  const loadProducts = () => {
    ...
    then((r) => {
      ...
      console.error(err);
      setError(t("Failed to load retail products"));
      setProducts([]);
    }).
    ...
    try {
      if (imageFile) {
        ...
        if (editId) {
          ...
          window.showToast(t("Retail product updated successfully"), 'success');
        } else {
          await api.post('/retail-products', formData, { headers });
          window.showToast(t("Retail product created successfully"), 'success');
        }
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      window.showToast(t("Retail product deleted successfully"), 'success');
      loadProducts();
    } catch (err) {
      window.showToast(t("Failed to delete retail product"), 'error');
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                    ...
                    <div className="relative">
                        ...
                        <input type="text" name="barcode" value={form.barcode} onChange={handleChange} placeholder={t("Enter or scan barcode")} className="w-full pl-10 pr-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none font-mono" />
                    </div>
                </div>
                <div>
                    <label className="block text-xs font-semibold text-gray-600 mb-1">{t("Product Name *")}</label>
                    <input type="text" name="name" value={form.name} onChange={handleChange} placeholder={t("e.g. Lipstick - Ruby Red")} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none" required />
                </div>
                <div>
                    <label className="block text-xs font-semibold text-gray-600 mb-1">{t("SKU")}</label>
                    <input type="text" name="sku" value={form.sku} onChange={handleChange} placeholder={t("e.g. COS-001")} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none" />
                </div>
                <div>
                ...
                <div>
                    <label className="block text-xs font-semibold text-gray-600 mb-1">{t("Selling Price *")}</label>
                    <div className="relative">
                ...
                <div className="md:col-span-2">
                    ...
                    <textarea name="description" value={form.description} onChange={handleChange} placeholder={t("Additional details about this product")} rows="2" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none" />
                </div>
                <div className="md:col-span-2">
                    <label className="block text-xs font-semibold text-gray-600 mb-1">{t("Product Image")}</label>
  ...
  return (
    <>
            <div className="mb-6 space-y-4">
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="relative flex-1">
                        ...
                        <input type="text" name="search" placeholder={t("Search by product name, SKU, or barcode...")} value={filters.search} onChange={handleSearchChange} className="w-full pl-11 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none" />
                    </div>
            ...
            <div className="flex justify-between items-center mb-5">
                <h3 className="text-base font-semibold text-gray-700">{t("All Retail Products (")}{products.length})</h3>
                {canCreate ?
        <button onClick={openCreate} className="btn-primary px-4 py-2 text-sm transition-colors flex items-center gap-2">
                        <Package size={16} />{t("Add New Retail Product")}
        </button> :
        ...
        <button onClick={() => navigate('/retail-otc-sales')} className="btn-primary px-4 py-2 text-sm transition-colors flex items-center gap-2">
                        <ShoppingBag size={16} />{t("Create Retail Sale")}
        </button> :
        null}
            </div>

            {loading ? <LoadingSpinner text={t("Loading retail products...")} /> :
      <>
                            ...
                            <table className="w-full table-fixed">
                                ...
                                <thead>
                                    <tr className="bg-sky-50 border-b border-sky-100">
                                        <th className="table-header">{t("Product Name")}</th>
                                        <th className="table-header">{t("Category")}</th>
                                        <th className="table-header">{t("Barcode")}</th>
                                        <th className="table-header">{t("Qty")}</th>
                                        <th className="table-header">{t("Selling Price")}</th>
                                        <th className="table-header">{t("Expiry Date")}</th>
                          ...
                          e.target.src = '/images/medicine-placeholder.svg';
                                            ...
                                            <td className="px-4 py-3 text-right">
                                                <div className="flex justify-end gap-1">
                                                    <button onClick={() => openView(p)} className="p-1.5 text-sky-600 hover:bg-sky-50 rounded transition-colors" title={t("View")}><Eye size={16} /></button>
                                                    {(canEdit || canDelete) &&
                      <>
                                                            {canEdit && <button onClick={() => openEdit(p)} className="p-1.5 text-sky-600 hover:bg-sky-50 rounded transition-colors" title={t("Edit")}><Edit size={16} /></button>}
                                                            {canDelete && <button onClick={() => handleDelete(p.id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded transition-colors" title={t("Delete")}><Trash2 size={16} /></button>}
                                                        </>
                      }
                                                </div>
                                            </td>
                ...
                <tr><td colSpan="8" className="px-4 py-8 text-center text-gray-400">{t("No retail products found")}
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editId ? 'Edit Retail Product' : 'Add New Retail Product'} size="max-w-2xl">
            ...
            <Modal open={showViewModal} onClose={() => setShowViewModal(false)} title={t("Retail Product Details")} size="max-w-3xl">
                {viewProduct &&
        <>
              ...
              <div className="w-32 h-32 rounded-xl bg-gray-100 flex items-center justify-center border border-gray-200">
                            ...
                            <div className="flex-1 flex flex-col justify-center">
                                ...
                                <p className="text-sm text-gray-500 mb-2">{t("SKU:")}{viewProduct.sku || '---'}</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">{t("Product Name")}</label><p className="text-sm font-medium text-gray-800">{viewProduct.name}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">{t("SKU")}</label><p className="text-sm text-gray-600">{viewProduct.sku || '---'}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">{t("Barcode")}</label><p className="text-sm font-mono text-gray-600">{viewProduct.barcode || '---'}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">{t("Category")}</label><p className="text-sm text-gray-600">{viewProduct.category || 'General'}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">{t("Supplier")}</label><p className="text-sm text-gray-600">{viewProduct.supplier?.name || 'No Supplier'}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">{t("Manufacturer")}</label><p className="text-sm text-gray-600">{viewProduct.manufacturer || '---'}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">{t("Status")}</label><div className="mt-1">{getStatusBadge(viewProduct.status)}</div></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">{t("Quantity")}</label><p className="text-sm font-medium text-gray-800">{viewProduct.quantity}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">{t("Reorder Level")}</label><p className="text-sm text-gray-600">{viewProduct.reorder_level}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">{t("Selling Price")}</label><p className="text-sm text-gray-600">{viewProduct.price ? `$${Number(viewProduct.price).toFixed(2)}` : '---'}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">{t("Purchase Price")}</label><p className="text-sm text-gray-600">{viewProduct.purchase_price ? `$${Number(viewProduct.purchase_price).toFixed(2)}` : '---'}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">{t("Expiry Date")}</label><p className="text-sm text-gray-600">{viewProduct.expiry_date ? new Date(viewProduct.expiry_date).toLocaleDateString() : '---'}</p></div>
                            <div className="md:col-span-2"><label className="block text-xs font-semibold text-gray-500 mb-1">{t("Description")}</label><p className="text-sm text-gray-600">{viewProduct.description || '---'}</p></div>
                        </div>
                        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-sky-100">
                            ...
                            {canEdit &&
            <button onClick={() => {setShowViewModal(false);openEdit(viewProduct);}} className="btn-primary px-4 py-2 text-sm flex items-center gap-2"><Edit size={16} />{t("Edit Product")}</button>
            }
                        </div>
                    </>
        }
