...
import { Search, Filter, Eye, Edit, Trash2, X, Save, Package, Tag, Barcode, Camera, Loader2, ShoppingBag } from 'lucide-react';
...
export default function RetailProducts() {
    ...
    const resetForm = () => {
        setForm({ 
            ...
            shelf_location: '', description: '', reorder_level: '10' 
        });
        setImageFile(null);
        setImagePreview('');
        setEditId(null); 
    ...
    return (
        <>
            <div className="mb-6 space-y-4">
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="relative flex-1">
                        ...
                        <input type="text" name="search" placeholder="Search by product name, SKU, or barcode..." value={filters.search} onChange={handleFilterChange} className="w-full pl-11 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none" />
                    </div>
                    <div className="relative w-full md:w-48">
                        <Tag className="absolute left-3 top-3 w-5 h-5 text-gray-400" />
                        <select name="category" value={filters.category} onChange={handleFilterChange} className="w-full pl-11 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none appearance-none bg-white">
                            <option value="">All Categories</option>
                            {categories.filter(c => c.type === 'retail' || c.type === 'otc').map(cat => (
                    ...
                    <div className="relative w-full md:w-48">
                        ...
                        <select name="supplier_id" value={filters.supplier_id} onChange={handleFilterChange} className="w-full pl-11 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none appearance-none bg-white">
                    ...
                    <div className="relative w-full md:w-48">
                        ...
                        <select name="status" value={filters.status} onChange={handleFilterChange} className="w-full pl-11 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none appearance-none bg-white">
            ...
            {(canCreate || canEdit) && (
                <Modal open={showModal} onClose={() => setShowModal(false)} title={editId ? 'Edit Retail Product' : 'Add New Retail Product'} size="max-w-xl">
                    {error && <div className="bg-red-50 text-red-600 px-4 py-3 rounded-xl mb-4 text-sm border border-red-100">{error}</div>}
                    <form onSubmit={handleSubmit} className="space-y-4">
                        {/* 1. Barcode */}
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">1. Barcode</label>
                        ...
                        {/* 2. Product Name */}
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">2. Product Name *</label>
                        ...
                        {/* 3. SKU */}
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">3. SKU</label>
                        ...
                        {/* 4. Category * */}
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">4. Category *</label>
                            <select name="category" value={form.category} onChange={handleChange} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none bg-white" required>
                        ...
                        {/* 5. Shelf Location */}
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">5. Shelf Location</label>
                        ...
                        {/* 6. Description */}
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">6. Description</label>
                        ...
                        {/* 7. Product Image */}
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">7. Product Image</label>
                        ...
                        {/* 8. Reorder Level */}
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">8. Reorder Level *</label>
            ...
            <Modal open={showViewModal} onClose={() => setShowViewModal(false)} title="Retail Product Details" size="max-w-xl">
                {viewProduct && (
                    <>
                            ...
                            <div className="flex-shrink-0">
                                {viewProduct.image_url ? (
                                    <img 
                                        ...
                                        className="w-24 h-24 rounded-xl object-cover border border-gray-200 shadow-sm bg-gray-50"
                                        onError={(e) => {
                                            e.target.onerror = null;
                                            e.target.src = '/images/medicine-placeholder.svg';
                                        }}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">Product Name</label><p className="font-medium text-gray-800">{viewProduct.name}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">SKU</label><p className="text-gray-600">{viewProduct.sku || '---'}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">Barcode</label><p className="font-mono text-gray-600">{viewProduct.barcode || '---'}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">Category</label><p className="text-gray-600">{viewProduct.category || 'General'}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">Shelf Location</label><p className="text-gray-600">{viewProduct.shelf_location || '---'}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">Reorder Level</label><p className="text-gray-600">{viewProduct.reorder_level}</p></div>
                            <div><label className="block text-xs font-semibold text-gray-500 mb-1">Quantity (Stock)</label><p className="font-bold text-gray-800">{viewProduct.quantity}</p></div>
                            <div className="md:col-span-2"><label className="block text-xs font-semibold text-gray-500 mb-1">Description</label><p className="text-gray-600">{viewProduct.description || '---'}</p></div>
                        </div>
