...
export default function SalesHistory() {const { t } = useLanguage();
  ...
  const fetchSales = async (page = 1) => {
    ...
    try {
      ...
      window.showToast(t("Failed to load sales history"), 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleViewReceipt = async (saleId) => {
    ...
    try {
      ...
      console.error(err);
      window.showToast(t("Failed to load sale details"), 'error');
    } finally {
  ...
  const handleExport = async (type, format) => {
    try {
      ...
      console.error(err);
      window.showToast(t("Failed to export sales report"), 'error');
    }
  };

  if (loading) return <LoadingSpinner text={t("Loading sales history...")} />;
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-gray-800">{t("Sales History")}</h2>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                <div className="flex flex-col lg:flex-row gap-4">
                    ...
                    <div className="flex-1 relative">
                        ...
                        <input
              type="text"
              placeholder={t("Search by receipt number or customer...")}
              value={searchTerm}
          ...
          <div className="w-full sm:w-40">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-gray-50 border-b border-gray-200">
                            <tr>
                                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t("Receipt Number")}</th>
                                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t("Date")}</th>
                                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t("Cashier")}</th>
                                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t("Customer")}</th>
                                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t("Payment Method")}</th>
                                <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{t("Total")}</th>
              ...
              <tr>
                                    <td colSpan="8" className="px-4 py-8 text-center text-gray-400">{t("No sales found. Try adjusting your search or filters.")}
        ...
        <div className="flex justify-between items-center px-4 py-3 border-t border-gray-200">
                        <div className="text-sm text-gray-500">{t("Page")}
        ...
        size="max-w-2xl">
                ...
                {modalLoading ?
        <LoadingSpinner text={t("Loading sale details...")} /> :
        selectedSale ?
        <div className="space-y-6">
                        {/* Header Badges */}
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-gray-100">
                            ...
                            <div className="flex gap-2">
                                ...
                                <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-sky-100 text-sky-700 capitalize">
                                    {selectedSale.type || 'Standard'}{t("Sale")}
              </span>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm bg-gray-50 p-4 rounded-xl border border-gray-100">
                            <div>
                                <span className="block text-xs font-semibold text-gray-400">{t("Cashier")}</span>
                                <p className="font-medium text-gray-800">{selectedSale.cashier_name || selectedSale.user?.name || 'Unknown'}</p>
                            </div>
                            <div>
                                <span className="block text-xs font-semibold text-gray-400">{t("Customer")}</span>
                                <p className="font-medium text-gray-800">{selectedSale.customer_name || 'Walk-in Customer'}</p>
                            </div>
                            {selectedSale.customer_phone &&
            <div>
                                    <span className="block text-xs font-semibold text-gray-400">{t("Customer Phone")}</span>
                                    <p className="text-gray-700">{selectedSale.customer_phone}</p>
                                </div>
            }
                            {selectedSale.customer_email &&
            <div>
                                    <span className="block text-xs font-semibold text-gray-400">{t("Customer Email")}</span>
                                    <p className="text-gray-700">{selectedSale.customer_email}</p>
                                </div>
            }
                            <div>
                                <span className="block text-xs font-semibold text-gray-400">{t("Payment Method")}</span>
                                <p className="font-medium text-gray-800">{PAYMENT_LABELS[selectedSale.payment_method] || selectedSale.payment_method || 'N/A'}</p>
                            </div>
                            <div>
                                <span className="block text-xs font-semibold text-gray-400">{t("Payment Status")}</span>
                        ...
                        {/* Purchased Items */}
                        <div>
                            <h5 className="text-sm font-semibold text-gray-700 mb-2">{t("Purchased Items (")}{selectedSale.items?.length || 0})</h5>
                  ...
                  <tr>
                                                <td colSpan="4" className="px-3 py-4 text-center text-gray-400">{t("No items in this sale")}</td>
                                            </tr>
                  }
                                    </tbody>
                                </table>
