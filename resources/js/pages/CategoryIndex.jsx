...
import {
  ...
  Search, Filter, X } from
'lucide-react';

export default function CategoryIndex() {const { t } = useLanguage();
  const { user } = useAuth();
  ...
  const loadCategories = () => {
    ...
    catch(() => setError(t("Failed to load categories"))).
  ...
  const loadShelves = () => {
    ...
    catch(() => setError(t("Failed to load shelves"))).
    finally(() => setLoading(false));
  };
            ...
            <div key={shelf.id} className="card p-5 hover:shadow-md transition-shadow">
                    ...
                    style={{ width: `${percent}%` }} />
                                    ...
                                    <div className="flex justify-between text-xs text-gray-500">
                                        ...
                                        <span>{t("Capacity:")}{shelf.capacity}</span>
                                    </div>
                                </div>
                            </div>);
