...
import {
  ...
  LayoutGrid, List, Pill, ChevronRight, Boxes, Building2 } from
'lucide-react';
...
export default function Categories() {const { t } = useLanguage();
  ...
  const loadShelves = () => {
    ...
    then(([branchRes, whRes]) => {
      ...
      if (branchRes.data && branchRes.data.meta) setMeta(branchRes.data.meta);else
      if (Array.isArray(branchRes.data)) setMeta(null);else
      setMeta(branchRes.data || null);
    }).
    catch((err) => {
      console.error(err);
      setError(t("Failed to load shelves"));
      setShelves([]);
      setWarehouseShelves([]);
    }).
    finally(() => setLoading(false));
  ...
  const openShelfItems = async (shelf) => {
    ...
    try {
      ...
      window.showToast(t("Failed to load medicines on this shelf"), 'error');
    } finally {
  ...
  const handleDeleteCategory = async (id) => {
    if (!canWrite) {
      window.showToast(t("Only admins and pharmacists can delete categories"), 'error');
      return;
    }
    if (!confirm(t("Delete this category? This will not delete associated medicines."))) return;
    try {
      await api.delete(`/categories/${id}`);
      window.showToast(t("Category deleted successfully"), 'success');
      loadCategories();
    } catch (err) {
  ...
  const handleDeleteShelf = async (id) => {
    if (!canWrite) {
      window.showToast(t("Only admins and pharmacists can delete shelves"), 'error');
      return;
    }
    if (!confirm(t("Delete this shelf? This will not delete associated medicines."))) return;
    try {
      await api.delete(`/shelves/${id}`);
      window.showToast(t("Shelf deleted successfully"), 'success');
      loadShelves();
      loadAllShelves();
    } catch (err) {
  ...
  const openCreate = (type) => {
    if (!canWrite) {
      window.showToast(t("Only admins and pharmacists can create"), 'error');
      return;
    }
    setModalType(type);
    setModalMode('create');
  ...
  const openEdit = (item, type) => {
    if (!canWrite) {
      window.showToast(t("Only admins and pharmacists can edit"), 'error');
      return;
    }
    setModalType(type);
    setModalMode('edit');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.status === 403) {
        window.showToast(t("You do not have permission to perform this action"), 'error');
        ...
        setError(t("Please fix the validation errors"));
      } else {
  ...
  <div className="card overflow-hidden">
                ...
                <table className="w-full">
                    <thead>
                        <tr className="bg-sky-50 dark:bg-gray-800 border-b border-sky-100 dark:border-gray-700">
                            <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700 dark:text-sky-400 uppercase">{t("Category Name")}</th>
                            <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700 dark:text-sky-400 uppercase">{t("Description")}</th>
                            <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700 dark:text-sky-400 uppercase">{t("Shelf Location")}</th>
                            <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700 dark:text-sky-400 uppercase">{t("Medicines")}</th>
                            <th className="px-4 py-3 text-right text-xs font-semibold text-sky-700 dark:text-sky-400 uppercase">{t("Actions")}</th>
                        </tr>
                    </thead>
          ...
          <tr>
                                <td colSpan="5" className="px-4 py-8 text-center text-gray-400">{t("No categories found")}

              {canWrite &&
              <button
                onClick={() => openCreate('category')}
                className="ml-2 text-sky-600 hover:underline text-sm font-medium">{t("Create one")}
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {list.map((shelf) => {const { t } = useLanguage();
      const capacity = shelf.capacity || 100;
        ...
        <div
          ...
          className="card p-5 hover:shadow-lg transition-all duration-200 flex flex-col justify-between group border border-sky-100 dark:border-gray-700">
              ...
              'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400'}`
                ...
                title={t("Inspect Items")}>
                
                                    <Eye size={14} />
                                    <span>{t("Items (")}{current})</span>
                                </button>
                                {canWrite &&
              <>
                                        <button
  ...
  <div className="card overflow-hidden">
                ...
                <table className="w-full">
                    <thead>
                        <tr className="bg-sky-50 dark:bg-gray-800 border-b border-sky-100 dark:border-gray-700">
                            <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700 dark:text-sky-400 uppercase">{t("Shelf Location")}</th>
                            <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700 dark:text-sky-400 uppercase">{t("Description")}</th>
                            <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700 dark:text-sky-400 uppercase">{t("Capacity")}</th>
                            <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700 dark:text-sky-400 uppercase">{t("Progress")}</th>
                            <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700 dark:text-sky-400 uppercase">{t("Status")}</th>
                            <th className="px-4 py-3 text-right text-xs font-semibold text-sky-700 dark:text-sky-400 uppercase">{t("Actions")}</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                        {shelves.length > 0 ? shelves.map((shelf) => {const { t } = useLanguage();
            const totalItems = shelf.current_items ?? (shelf.medicines_count || 0);
            const capacity = shelf.capacity || 100;
            const percent = shelf.utilization ?? Math.min(100, Math.round(totalItems / capacity * 100));
            const progressColor = getProgressColor(percent);
            ...
            return (
              <tr key={shelf.id} className="hover:bg-sky-50/30 dark:hover:bg-gray-800/50 transition-colors">
                      ...
                      onClick={() => openShelfItems(shelf)}
                      className="p-1.5 text-sky-600 hover:bg-sky-50 dark:hover:bg-gray-700 rounded transition-colors"
                      title={t("Inspect Shelf Medicines")}>
          ...
          <tr>
                                <td colSpan="6" className="px-4 py-8 text-center text-gray-400">{t("No shelves found")}
  ...
  <div className="space-y-4">
            ...
            <div className="flex justify-between items-center bg-white dark:bg-gray-800 p-3 rounded-xl border border-sky-100 dark:border-gray-700">
                <div className="text-xs text-gray-500 dark:text-gray-400 font-medium">{t("Showing")}
        <span className="font-bold text-gray-800 dark:text-gray-200">{shelves.length}</span>{t("warehouse & pharmacy shelf locations")}
      </div>
          ...
          title={t("Card Grid View (Warehouse Style)")}>
    ...
    <div className="space-y-8">
      ...
      <section>
                            <h3 className="text-sm font-bold text-gray-700 dark:text-gray-200 mb-3 flex items-center gap-2">
                                <Building2 size={16} className="text-amber-600" />{t("Central Warehouse Shelves")}
        ...
        <div className="text-center py-8 text-gray-400">{t("No branch shelves found")}</div>
        }
                    </section>
                </div> :
  ...
  if (loading && categories.length === 0 && shelves.length === 0) {
    return <LoadingSpinner text={t("Loading Categories & Shelves...")} />;
  }
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-white">{t("Categories & Shelves")}</h2>
        ...
        <button
          ...
          className="btn-primary px-4 py-2 text-sm transition-colors flex items-center gap-2">
                        ...
                        <Plus size={16} />{t("Add")}
          {activeTab === 'categories' ? 'Category' : 'Shelf'}
                    </button>
        }
            </div>
      ...
      <Pagination meta={meta} onPageChange={(p) => setPage(p)} />
        ...
        <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="p-3.5 bg-sky-50 dark:bg-gray-800 rounded-xl border border-sky-100 dark:border-gray-700">
                                <p className="text-xs text-gray-500 dark:text-gray-400">{t("Capacity Load")}</p>
                                <p className="text-sm font-bold text-gray-900 dark:text-white flex items-center gap-1.5 mt-0.5">
                                    <Boxes size={14} className="text-emerald-500" />
                                    {selectedShelfDetail.item_count || 0}{t("products \xB7")}{selectedShelfDetail.total_items || 0} units
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
                            <table className="w-full">
                                <thead className="bg-gray-50 dark:bg-gray-800">
                                    <tr>
                                        <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-600 dark:text-gray-300">{t("Medicine")}</th>
                                        <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-600 dark:text-gray-300">{t("Category")}</th>
                                        <th className="px-4 py-2.5 text-center text-xs font-semibold text-gray-600 dark:text-gray-300">{t("Quantity")}</th>
                                        <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-600 dark:text-gray-300">{t("Status")}</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                <tr>
                                            <td colSpan="4" className="px-4 py-8 text-center text-gray-400">{t("No medicines assigned to this shelf yet")}
              ...
              className="btn-secondary">{t("Close")}
        ...
        <div className="space-y-4">
                        ...
                        {modalType === 'shelf' &&
          <div>
                                <label className="block text-xs font-semibold text-gray-500 mb-1">{t("Capacity")}</label>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <>
                                <div>
                                    <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">{t("Category Name *")}</label>
                                    <input
                name="name"
                value={form.name}
              ...
              <p className="text-xs text-red-500 mt-1">{validationErrors.name[0]}</p>
                                ...
                                <div>
                                    <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">{t("Shelf Location")}</label>
                                    <select
                name="shelf_location"
                ...
                className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none">
                                        ...
                                        <option value="">{t("Select Shelf")}</option>
                                        {allShelves.length > 0 ?
                allShelves.map((shelf) =>
                <option key={shelf.id} value={shelf.shelf_location}>
                                                    {shelf.shelf_location}{t("(Capacity:")}{shelf.capacity || 100})
                                                </option>
                ) :

                <option value="" disabled>{t("No shelves available")}</option>
                }
                                    </select>
              ...
              <p className="text-xs text-red-500 mt-1">{validationErrors.shelf_location[0]}</p>
                                    ...
                                    {allShelves.length === 0 &&
              <p className="text-xs text-amber-500 mt-1">{t("No shelves found. Please create a shelf first.")}
          ...
          <>
                                <div>
                                    <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">{t("Shelf Name *")}</label>
                                    <input
                name="name"
                value={form.name}
                onChange={handleChange}
                placeholder={t("e.g. Medicine Shelf A")}
                className={`w-full px-3 py-2 border rounded-lg text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none ${
                validationErrors.name ? 'border-red-400 focus:border-red-400' : 'border-gray-200 dark:border-gray-700'}`
                }
                required />
              ...
              <p className="text-xs text-red-500 mt-1">{validationErrors.name[0]}</p>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">{t("Shelf Code *")}</label>
                                        <input
                  name="code"
                  value={form.code}
                  onChange={handleChange}
                  placeholder={t("e.g. MSA-01")}
                  className={`w-full px-3 py-2 border rounded-lg text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none ${
                ...
                <p className="text-xs text-red-500 mt-1">{validationErrors.code[0]}</p>
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">{t("Capacity *")}</label>
                                        <input
                  type="number"
                  name="capacity"
                  value={form.capacity}
                ...
                <p className="text-xs text-red-500 mt-1">{validationErrors.capacity[0]}</p>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">{t("Location Type *")}</label>
                                        <select
                  name="location_type"
                  ...
                  className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none">
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">{t("Product Type *")}</label>
                                        <select
                  name="product_type"
                  ...
                  className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:border-sky-400 focus:ring-2 focus:ring-sky-100 outline-none">
                                            ...
                                            <option value="retail_otc">{t("Retail & OTC")}</option>
                                        </select>
                                    </div>
                                </div>
                                {form.location_type === 'branch' &&
            <div>
                                        <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">{t("Branch *")}</label>
                                        <select
                name="branch_id"
            ...
            <div className="p-3 rounded-lg bg-amber-50 border border-amber-200 text-xs text-amber-700">{t("This is a")}
              <strong>{t("Warehouse")}</strong>{t("shelf. It is not assigned to any branch and can only hold received warehouse stock.")}
            </div>
            }
                            </>
          }
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">{t("Description")}</label>
                            <textarea
              name="description"
            ...
            <p className="text-xs text-red-500 mt-1">{validationErrors.description[0]}</p>
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            <button type="button" onClick={closeModal} className="btn-secondary">{t("Cancel")}
