...
export default function RolesPermissions() {const { t } = useLanguage();
  ...
  const loadRoles = async () => {
    ...
    try {
      ...
      setError(t("Failed to load roles & permissions"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    if (!form.name.trim()) {
      window.showToast(t("Role name is required"), "error");
      return;
    }
    setSubmitting(true);
    try {
      ...
      if (editingRole) {
        ...
        window.showToast(t("Role updated successfully"), "success");
      } else {
        await api.post("/roles", payload);
        window.showToast(t("Role created successfully"), "success");
      }
      setShowModal(false);
      loadRoles();
    } catch (err) {
  ...
  const handleDelete = async (role) => {
    ...
    try {
      ...
      window.showToast(t("Role deleted successfully"), "success");
      loadRoles();
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
                <LoadingSpinner text={t("Loading roles...")} />
            </div>);
      ...
      <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm">
            ...
            <div className="flex items-center justify-between">
                <p className="text-sm text-gray-500">
                    {roles.length} role{roles.length !== 1 ? "s" : ""} · {permissions.length}{t("permissions available")}
        </p>
        ...
        <div
          ...
          className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow p-5 flex flex-col">
              ...
              "bg-violet-100 text-violet-600"}`
                            ...
                            {role.is_system &&
            <span className="px-2 py-0.5 rounded-md bg-gray-100 text-gray-500 text-[10px] font-semibold uppercase tracking-wider">{t("System")}
                        ...
                        <div className="mt-4 flex items-center gap-2 text-sm">
                            ...
                            {role.slug === "admin" &&
            <span className="px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 font-semibold text-xs">{t("Full access (all permissions)")}
        ...
        size="max-w-2xl">
                ...
                <div className="p-5 space-y-5 overflow-y-auto">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1.5">{t("Role Name")}</label>
                            <input
                type="text"
                name="name"
                value={form.name}
                ...
                onChange={handleChange}
                placeholder={t("What is this role responsible for?")}
                className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100" />
                    ...
                    <div>
                        ...
                        {groups.map((group) => {const { t } = useLanguage();
              const groupSlugs = permissions.filter((p) => p.group === group).map((p) => p.slug);
        ...
        <div className="p-5">
                        <p className="text-sm text-gray-500 mb-4">
                            ...
                            {showView.permissions?.length || 0}{t("permissions granted.")}
          </p>
                        {showView.slug === "admin" &&
          <div className="bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-3 rounded-xl text-sm mb-4">{t("Admins automatically have every permission in the system.")}
            ...
            <p className="text-sm text-gray-400">{t("No permissions assigned yet.")}</p>
            }
                        </div>
                    </div>
        }
