...
import {
  ...
  Upload, ClipboardCheck, RotateCcw, Search, Pill } from
'lucide-react';
 ...
 */
const ActionIcon = ({ icon: Icon, tooltip, color = "sky", onClick, disabled = false }) => {const { t } = useLanguage();
  const colorClasses = {
...
export default function PurchaseOrders() {const { t } = useLanguage();
  ...
  const load = async (pageNumber = page) => {
    try {
      ...
      console.error(err);
      setError(t("Failed to load purchase orders"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!confirm(t("Delete this purchase order?"))) return;
    try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modalMode === 'create') {
        ...
        window.showToast(t("Purchase order created successfully"), 'success');
        if (page !== 1) {
          setPage(1);
        } else {
          load(1);
  ...
  return (
    <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h3 className="text-base font-semibold text-gray-700">{t("All Purchase Orders (")}
      ...
      <div className="bg-red-50 text-red-700 p-3 rounded-lg text-sm">
            ...
            {loading ?
      <LoadingSpinner text={t("Loading purchase orders...")} /> :
      ...
      <div className="card overflow-hidden">
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-sky-50">
                                    ...
                                    <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700">{t("Supplier")}</th>
                                    <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700">{t("Date")}</th>
                                    <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700">{t("Status")}</th>
                                    <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700">{t("Sent At")}</th>
                                    <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700">{t("Delivered At")}</th>
                                    <th className="px-4 py-3 text-left text-xs font-semibold text-sky-700">{t("Items")}</th>
                                    <th className="px-4 py-3 text-right text-xs font-semibold text-sky-700">{t("Actions")}</th>
                                </tr>
                            </thead>
                            <tbody>
                                {orders.map((o, idx) => {const { t } = useLanguage();
                    ...
                    className="border-b hover:bg-sky-50/30">
                        ...
                        <ActionIcon
                          ...
                          disabled={a.disabled} />
                                ...
                                {orders.length === 0 &&
              <tr>
                                        <td colSpan="8" className="px-4 py-8 text-center text-gray-400">{t("No purchase orders found")}
        ...
        title={t("Purchase Order PDF Preview")}
        size="max-w-4xl">
        
                {pdfLoading ?
        <div className="flex justify-center py-12">
                        <LoadingSpinner text={t("Generating PDF...")} />
                    </div> :
        pdfPreviewData ?
        ...
        <p className="text-gray-500">{t("No PDF data available.")}</p>
        }
            </Modal>
        ...
        <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-gray-500 mb-1">{t("Sent At")}</label>
                                <p className="text-sm text-gray-600">
                                    {modalItem.sent_at_display || 'Not sent yet.'}
                                </p>
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-gray-500 mb-1">{t("Delivered At")}</label>
                                <p className="text-sm text-gray-600">
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-xs font-semibold text-gray-500 mb-1">{t("Order Items (")}{modalItem.items?.length || 0})</label>
                                <div className="border border-gray-200 rounded-lg overflow-hidden">
                                    <table className="w-full">
                                        ...
                                        <tbody>
                                            {(modalItem.items || []).map((oi, i) => {const { t } = useLanguage();
                      const isRetail = oi.itemable_type?.includes('RetailProduct');
                        ...
                        title={t("Remove item")}>
                                        ...
                                        <tfoot>
                                            <tr className="border-t-2 border-gray-200 bg-gray-50">
                                                <td colSpan="2" className="px-4 py-2 text-right text-xs font-bold text-gray-700">{t("Total Items:")}</td>
