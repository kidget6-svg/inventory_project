...
import {
  ...
  TrendingDown, RefreshCcw, Pill } from
'lucide-react';
...
export default function StockMovements() {const { t } = useLanguage();
  ...
  const loadMovements = useCallback(() => {
    ...
    then((r) => {
      ...
      setError(t("Failed to load stock movements"));
      setHasError(true);
      setMovements([]);
    }).
  ...
  const addItem = (type, product) => {
    ...
    if (selectedItems.some((item) => item.type === type && item.id === product.id)) {
      window.showToast(t("Item already added"), 'info');
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedItems.length === 0) {
      setError(t("Please add at least one product to the movement."));
      return;
    }
    ...
    if (invalidItems.length > 0) {
      setError(t("Please enter valid quantities for all items."));
      return;
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      window.showToast(t("Stock movement deleted"), 'success');
      loadMovements();
      loadMedicinesForSelect();
    } catch (err) {
  ...
  const handleExport = () => {
    window.showToast(t("Preparing PDF export..."), 'info');
  };
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-gray-800">{t("Stock Movements")}</h2>
                    <p className="text-sm text-gray-500 mt-1">{t("Track and manage inventory movements")}</p>
                </div>
                <div className="flex items-center gap-3 flex-wrap">
                    <button onClick={loadMovements} className="p-2.5 rounded-xl border border-gray-200 bg-white hover:bg-gray-50 text-gray-600 transition-all duration-200 hover:shadow-sm" title={t("Refresh")}>
                        <RefreshCw size={18} />
                    </button>
            ...
            {kpiStats &&
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-11 gap-3">
                    <StatCard value={kpiStats.total} label={t("Total")} icon="package" color="blue" />
                    <StatCard value={kpiStats.in} label={t("Stock In")} icon="trending-up" color="green" />
                    <StatCard value={kpiStats.out} label={t("Stock Out")} icon="trending-down" color="red" />
                    <StatCard value={kpiStats.adjustments} label={t("Adjustments")} icon="clipboard-list" color="orange" />
                    <StatCard value={kpiStats.transfers} label={t("Transfers")} icon="truck" color="purple" />
                    <StatCard value={kpiStats.returns} label={t("Returns")} icon="rotate-ccw" color="sky" />
                    <StatCard value={kpiStats.damaged} label={t("Damaged")} icon="alert" color="red" />
                    <StatCard value={kpiStats.expired} label={t("Expired")} icon="calendar" color="gray" />
                    <StatCard value={kpiStats.lost} label={t("Lost")} icon="alert" color="orange" />
                    <StatCard value={kpiStats.self} label={t("Self")} icon="users" color="teal" />
                    <StatCard value={kpiStats.totalQuantity} label={t("Qty Moved")} icon="activity" color="blue" />
                </div>
      }
      ...
      <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-center justify-between">
                    ...
                    <button onClick={handleRetry} className="px-4 py-2 bg-red-600 text-white rounded-xl text-sm font-semibold hover:bg-red-700 transition-colors">{t("Retry")}
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              {activeFilterCount > 0 && <span className="bg-sky-500 text-white text-xs px-1.5 py-0.5 rounded-full">{activeFilterCount}</span>}
                        ...
                        {activeFilterCount > 0 &&
            <button onClick={clearFilters} className="px-3 py-2.5 rounded-xl text-sm text-red-600 hover:bg-red-50 transition-colors">{t("Clear")}
        ...
        <div className="p-4 border-b border-gray-100 bg-gray-50/50 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        <div>
                            ...
                            <select value={filters.medicine_id} onChange={(e) => setFilters({ ...filters, medicine_id: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:border-sky-400 outline-none bg-white">
                                <option value="">{t("All Medicines")}</option>
                                {medicines.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}
                            </select>
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">{t("Type")}</label>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-gradient-to-r from-sky-600 to-blue-600 text-white">
                                ...
                                <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">{t("Medicine")}</th>
                                <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">{t("Type")}</th>
                                <th className="px-4 py-3 text-center text-xs font-bold uppercase tracking-wider">{t("Qty")}</th>
                                <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">{t("Before")}</th>
                                <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">{t("After")}</th>
                                <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">{t("User")}</th>
                                <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">{t("Reference")}</th>
                                <th className="px-4 py-3 text-right text-xs font-bold uppercase tracking-wider">{t("Actions")}</th>
                            </tr>
                        </thead>
              ...
              <tr>
                                    <td colSpan="9" className="px-4 py-12 text-center">
                                        <div className="flex flex-col items-center gap-3">
                                            ...
                                            <p className="text-gray-500 font-medium">{t("No stock movements recorded")}</p>
                                            <button onClick={openCreate} className="text-sm text-sky-600 font-semibold hover:underline">{t("Record your first movement")}</button>
                                        </div>
                                    </td>
                                </tr> :
              movements.map((m) => {const { t } = useLanguage();
                  ...
                  <tr key={m.id} className={`hover:bg-sky-50/30 transition-colors duration-150 cursor-pointer ${isSelf ? 'bg-teal-50/20' : ''}`} onClick={() => openView(m)}>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                                                <button onClick={() => openView(m)} className="p-2 text-sky-600 hover:bg-sky-50 rounded-lg transition-colors" title={t("View")}>
                                                    <Eye size={16} />
                                                </button>
                                                <button onClick={() => handleDuplicate(m)} className="p-2 text-gray-500 hover:bg-gray-50 rounded-lg transition-colors" title={t("Duplicate")}>
                                                    <Copy size={16} />
                                                </button>
                                                <button onClick={() => handleDelete(m.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors" title={t("Delete")}>
                                                    <Trash2 size={16} />
                                                </button>
                                            </div>
                                        </td>
        ...
        <div className="space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-semibold text-gray-500 mb-1">{t("Type")}</label>
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-gray-500 mb-1">{t("Before Stock")}</label>
                                <p className="text-sm text-gray-600">{modalItem.before_quantity ?? '---'}</p>
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-gray-500 mb-1">{t("After Stock")}</label>
                                <p className="text-sm text-gray-600">{modalItem.after_quantity ?? '---'}</p>
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-gray-500 mb-1">{t("Source")}</label>
                                <p className="text-sm text-gray-600">{modalItem.source_type || '---'}</p>
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-gray-500 mb-1">{t("Destination")}</label>
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            ...
                            <button onClick={closeModal} className="px-4 py-2 text-sm font-semibold text-white bg-sky-600 rounded-xl hover:bg-sky-700 transition-colors">{t("Close")}</button>
                        </div>
                    </div> :
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">{t("Products * (Add one or more)")}</label>
                            {/* Tab toggle */}
            ...
            <div className="relative" ref={medSearchRef}>
                  ...
                  placeholder={t("Search medicines by name, generic, or barcode...")} />
                
                                    </div>
                                    {showMedDropdown &&
              <div className="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-xl max-h-64 overflow-y-auto">
                                            {formLoading ?
                <div className="px-4 py-8 text-center text-gray-400 text-sm">{t("Loading medicines...")}</div> :
                ...
                <>
                  ...
                  <button
                    ...
                    className={`w-full text-left px-3 py-2.5 text-sm flex items-center justify-between hover:bg-sky-50 transition-colors ${isItemAdded('medicine', m.id) ? 'opacity-50 cursor-not-allowed' : ''}`}>
                                                            ...
                                                            <div className="flex items-center gap-3">
                                                                <span className="text-xs text-gray-500">{t("Stock:")}{m.quantity ?? 0}</span>
                  ...
                  <div className="px-4 py-8 text-center text-gray-400 text-sm">{t("No medicines found")}</div>
                  }
            ...
            <div className="relative" ref={retailSearchRef}>
                  ...
                  placeholder={t("Search retail products by name, SKU, or barcode...")} />
                
                                    </div>
                                    {showRetailDropdown &&
              <div className="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-xl max-h-64 overflow-y-auto">
                                            {formLoading ?
                <div className="px-4 py-8 text-center text-gray-400 text-sm">{t("Loading products...")}</div> :
                ...
                <>
                  ...
                  <button
                    ...
                    className={`w-full text-left px-3 py-2.5 text-sm flex items-center justify-between hover:bg-amber-50 transition-colors ${isItemAdded('retail', r.id) ? 'opacity-50 cursor-not-allowed' : ''}`}>
                                                            ...
                                                            <div className="flex items-center gap-3">
                                                                <span className="text-xs text-gray-500">{t("Stock:")}{r.quantity ?? 0}</span>
                  ...
                  <div className="px-4 py-8 text-center text-gray-400 text-sm">{t("No retail products found")}</div>
                  }
                                                    {filteredRetail.length > 50 &&
                  <div className="px-3 py-2 text-center text-xs text-gray-400 border-t border-gray-100">{t("Showing first 50 of")}
                    {filteredRetail.length} results.
                                                        </div>
                  }
                                                </>
                }
          ...
          <div>
                                <label className="block text-xs font-semibold text-gray-600 mb-1">{t("Selected Items (")}{selectedItems.length})</label>
                                <div className="border border-gray-200 rounded-lg overflow-hidden">
                                    <table className="w-full">
                                        <thead>
                                            <tr className="bg-gray-50">
                                                <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600">#</th>
                                                <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600">{t("Product")}</th>
                                                <th className="px-3 py-2 text-center text-xs font-semibold text-gray-600">{t("Type")}</th>
                                                <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600">{t("Manufacturer")}</th>
                                                <th className="px-3 py-2 text-right text-xs font-semibold text-gray-600">{t("Qty")}</th>
                  ...
                  <tr key={`${item.type}-${item.id}`} className="border-t border-gray-100">
                      ...
                      <Pill size={14} className="text-sky-400" />
                                                        ...
                                                        {item.name}
                                                        <span className="text-xs text-gray-400 ml-1">{t("(Stock:")}{item.stock})</span>
                                                    </td>
                      ...
                      "px-2 py-0.5 rounded-full text-xs font-semibold bg-sky-100 text-sky-700"
                        ...
                        className="p-1 text-red-500 hover:text-red-700 hover:bg-red-50 rounded transition-colors">
                                        ...
                                        <tfoot>
                                            <tr className="border-t-2 border-gray-200 bg-gray-50">
                                                <td colSpan="3" className="px-3 py-2 text-right text-xs font-bold text-gray-700">{t("Total Items:")}</td>
                        ...
                        {/* Movement Type */}
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">{t("Movement Type *")}</label>
                            <select
              name="type"
              value={form.type}
              ...
              <option key={key} value={key}>{cfg.label}</option>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <label className="block text-xs font-semibold text-gray-600 mb-1">{t("Source Type")}</label>
                             ...
                             <div>
                                 <label className="block text-xs font-semibold text-gray-600 mb-1">{t("Destination Type")}</label>
          ...
          <div>
                                <label className="block text-xs font-semibold text-gray-600 mb-1">{t("Source Branch *")}</label>
                                <select name="source_branch_id" value={form.source_branch_id} onChange={handleChange} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-sky-400 outline-none bg-white" required>
                                    <option value="">{t("Select Source Branch")}</option>
          ...
          <div>
                                <label className="block text-xs font-semibold text-gray-600 mb-1">{t("Destination Branch *")}</label>
                                <select name="destination_branch_id" value={form.destination_branch_id} onChange={handleChange} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-sky-400 outline-none bg-white" required>
                                    <option value="">{t("Select Destination Branch")}</option>
                                    {branches.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
                                </select>
                            </div>
          }
                        ...
                        {/* Reference & Notes */}
                        <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">{t("Reference")}</label>
                            <div className="relative">
              ...
              className="px-5 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-sky-600 to-blue-600 rounded-xl shadow-lg shadow-sky-500/20 hover:shadow-xl transition-all duration-300 flex items-center gap-2 disabled:opacity-60">
                                ...
                                {submitting ? <><Loader2 size={16} className="animate-spin" /> Recording...</> : <><Save size={16} />{t("Record Movement (")}{selectedItems.length}{t("items)")}</>}
                            </button>
                        </div>
                    </form>
        }
